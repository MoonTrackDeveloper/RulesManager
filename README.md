...
| Rules library | About |
|---|---|
| ![Rules library main screen](assets/rules-library-en.png?v=20260710-2) | ![About page](assets/about-en.png?v=20260710-2) |

| File | Platform | Description |
|------|----------|-------------|
| `Rules Manager_*_x64-setup.exe` | Windows 10/11 x64 | NSIS installer |
| `Rules Manager_*_aarch64.dmg` | macOS 10.15+ (Apple Silicon) | DMG with `.app` |
...
3. Launch from Applications (or Spotlight). If macOS blocks the first launch, open **System Settings → Privacy & Security** and allow **Rules Manager** to open
...
The app is not notarized by Apple. macOS may block the first launch — allow it under **System Settings → Privacy & Security** (exact wording varies by macOS version). Download only from Releases on this repository.
...
| 规则库 | 关于页 |
|---|---|
| ![规则库主界面](assets/rules-library-zh.png?v=20260710-2) | ![关于页](assets/about-zh.png?v=20260710-2) |

| 文件 | 平台 | 说明 |
|------|------|------|
| `Rules Manager_*_x64-setup.exe` | Windows 10/11 x64 | NSIS 安装包 |
| `Rules Manager_*_aarch64.dmg` | macOS 10.15+（Apple Silicon） | DMG（含 `.app`） |
...
3. 从「应用程序」或 Spotlight 启动；若首次打开被系统拦截，请在 **系统设置 → 隐私与安全性** 中允许 **Rules Manager** 打开
...
应用未经 Apple 公证。首次启动时 macOS 可能拦截 — 请在 **系统设置 → 隐私与安全性** 中允许打开（具体文案因系统版本略有不同）。请仅从本仓库 Releases 下载。
